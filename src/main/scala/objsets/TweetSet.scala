...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    val leftMostRetweeted = Try(left.mostRetweeted).getOrElse(elem)
    val rightMostRetweeted = Try(right.mostRetweeted).getOrElse(elem)
    if(leftMostRetweeted.retweets > elem.retweets && leftMostRetweeted.retweets > rightMostRetweeted.retweets) leftMostRetweeted
    else if(rightMostRetweeted.retweets > elem.retweets && rightMostRetweeted.retweets > leftMostRetweeted.retweets) rightMostRetweeted
    else elem
  }
